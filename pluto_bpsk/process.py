...
#detect if the samples received by the SDR contain a signal based on how much of the PSD is above a certain threshold
...
#check if the phase has been shifted during transmissin, if not returns the same samples. Otherwise, switch all ones to zeros, and zeros to ones.
...
#finds the index of the begining or the end of the data, is_start should equal True if looking for the start index.
...
is_signal = detect_signal(rx_samples, 2, 40)
print(is_signal)
...
if is_signal == True:
    ...
    corrected_samples = detect_phase(corrected_samples)
    ...
    bits = samples_to_ascii(corrected_samples, start_index, end_index)
    text = ascii_to_text(bits)
